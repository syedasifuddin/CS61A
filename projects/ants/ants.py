    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.is_container and self.ant.can_contain(insect) and not insect.is_container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        myPlace = self.place
        position = 0
        while myPlace.entrance != hive:
            if position >= self.min_range and position <= self.max_range:
                if myPlace.bees:
                    # self.place.bees = myPlace.bees
                    break
            myPlace = myPlace.entrance
            position += 1
        return random_or_none(myPlace.bees)
        # END Problem 3 and 4
